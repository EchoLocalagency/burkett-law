...
- [x] 02-03-PLAN.md — Homepage `/index.html` with hero + CTA trio, 8-card practice grid, Meet Brian teaser, 4-step how-it-works, 6-Q FAQ (visible + FAQPage schema), San Diego County service-area block, bottom CTA trio, LegalService+LocalBusiness+FAQPage+WebSite `@graph`, plus sitewide nav-path cutover from Phase 1 placeholders (`/attorney-bio/`, `/contact/`) to Phase 2 URLs (`/about.html`, `/contact.html`) (HOME-01..09)
